...
data = [list(map(float,line.split())) for line in open("files/5_A__4x393.txt")]
...
print(x*777,y*777)
data = [list(map(float,line.split())) for line in open("files/5_B__4x397.txt")]
clasters = clasterizasion(data,20)
visual(clasters)
centroid = [get_centroid(c) for c in clasters]
x, y = sum(x[0] for x in centroid) / len(centroid), sum(x[1] for x in centroid) / len(centroid)
print(x*777,y*777)
