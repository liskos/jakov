...
data = [list(map(float,line.split())) for line in open("files/27_4_A__51uc4.txt")]
...
print(x*333,y*666)
data = [list(map(float,line.split())) for line in open("files/27_4_B__51uc6.txt")]
clasters = clasterizasion(data,0.2)
centroid = [get_centroid(c) for c in clasters]
x, y = sum(x[0] for x in centroid) / len(centroid), sum(x[1] for x in centroid) / len(centroid)
visual(clasters)
print(x*321,y*123)
