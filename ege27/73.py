...
def visual(clasters):
    turtle.up()
    turtle.tracer(0)
    colors = ["red","yellow","black","yellow","orange","pink"]
...
data = [list(map(float,line.split()))for line in open("27data/27-73a.txt")]
clasters = clasterizasion(data,0.8)
clasters = [c for c in clasters if len(c) > 0]
...
data = [list(map(float,line.split()))for line in open("27data/27-73b.txt")]
