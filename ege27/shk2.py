...
def visual(clasters):
    turtle.up()
    turtle.tracer(0)
    colors = ["red","black","orange","purple","pink","blue"]
...
data = [list(map(float,line.split()))for line in open("27data/4_A__5t653.txt")]
...
data = [list(map(float,line.split()))for line in open("27data/2B__5gsp4.txt")]
clasters = clasterizasion(data,1)
clasters = [c for c in clasters if len(c) > 10]
visual(clasters)
