...
def visual(clasters):
    turtle.up()
    turtle.tracer(0)
    colors = ["red","blue","yellow","black","purple","orange","green"]
...
data = [list(map(float,line.split()))for line in open("27data/27-46a.txt")]
...
data = [list(map(float,line.split()))for line in open("27data/27-46b.txt")]
clasters = clasterizasion(data,1)
visual(clasters)
centroid = [get_centroid(c)for c in clasters]
x,y = sum(x[0] for x in centroid)/len(centroid),sum(x[1] for x in centroid) / len(centroid)
