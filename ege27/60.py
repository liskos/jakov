...
def visual(clasters):
    turtle.up()
    turtle.tracer(0)
    colors = ["red","blue","orange","yellow","purple","grey","pink"]
...
data = [list(map(float,line.split()))for line in open("27data/27-60a.txt")]
...
data = [list(map(float,line.split()))for line in open("27data/27-60b.txt")]
clasters = clasterizasion(data,0.8)
clasters = [c for c in clasters if len(c) > 10]
visual(clasters)
centroid = [get_centroid(c)for c in clasters]
