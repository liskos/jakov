...
def visual(clasters):
    turtle.up()
    turtle.tracer(0)
    colors = ["red","blue","yellow","grey","purple","orange"]
...
data = [list(map(float,line.split()))for line in open("27A_19083.txt")]
...
data = [list(map(float,line.split()))for line in open("27B_19083.txt")]
clasters = clasterizasion(data,1.4)
visual(clasters)
